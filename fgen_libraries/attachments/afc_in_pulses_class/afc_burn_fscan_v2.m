...
% 10/23/2015 updated from v1 to create a comb in a simpler way -jk
%% to do
...
fgen1.amp = 1;
fgen1.off = 0;
%set offset to 0.5 if you're using pulse mode...
%fgen1.off = .5;

readamp = .4; % amplitude of read pulse
...
totaltime = .25; %total experiment time in seconds.

%%%% afc parameters
nwriteseq = 20; % number of write sequences.
writelength = 2500; % length of write sequence (us)

edgepit = 200;
toothwidth = 50;
finesse = 4; %finesse = fsr/toothwidth;
...
%npulses = 2;
% writepulse_widths = writelength;%[50 repmat(50,1,npulses-2) 50]; % reminder: delays are specified in reference to the last pulse.
% writepulse_heights = ones(1,npulses);
% writepulse_delays = 1;%[10  repmat(100 ,1,npulses-1)]; %(us)
% %writepulse

% writepulse_widths =[20 10 5 5]; % reminder: delays are specified in reference to the last pulse.
% writepulse_heights = 1;
% writepulse_delays =[250 50 40 30]; %(us)


nreads = 1;

% note this is just for plotting. I currently use the built in pulse
% function
readlength = writelength;
readpulse_delays =0;
readpulse_widths = readlength*nreads;
readpulse_heights = 1;
%readpulse_rep = 10;

waitdelays_exp = 1e-6;
waitdelays = (1)*writelength; %delay between burning pulse and read pulse (units determined by waitdelays_exp)
%NOTE: THIS SHOULD BE IN MULTIPLES OF THE WRITELENGTH. otherwise, your scan
%will be off...

totaldelays = waitdelays*waitdelays_exp + nwriteseq*writelength*fgen1.pulses.timeexp; % this is in seconds!


totalnscans =  ceil(waitdelays.*waitdelays_exp/(writelength*fgen1.pulses.timeexp)) + nwriteseq+ nreads;
...
% comb.widths = [edgepit toothwidth*ones(1,nteeth) edgepit];
% comb.delays = [edgetopit pittocomb fsr*ones(1,nteeth-1) pittocomb];
% comb.heights = ones(1,nteeth+2);
...
% fgen1.send('FUNCtion PULSe');
% fgen1.send(sprintf('PULSe:PERiod %g',(writelength+10)*fgen1.pulses.timeexp));
% fgen1.send(sprintf('PULSe:WIDTh %g',writelength*fgen1.pulses.timeexp));

%

fgen2.send(sprintf('SOURce2:FREQuency %8.5G',1/(readlength*fgen1.pulses.timeexp)))
rampsym = 0;  
...
%fgen2.ch2.trigdelay = totaldelays(1);
fgen2.ch2.trigdelay = 0;
...
%     %set this to 0 if you want to scan during the write pulse.
%     %fgen2.ch2.trigdelay = 0;
%     %set this to totaldelays(it) if you want to burn at the center freq.
