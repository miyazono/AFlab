%% afc_burn_pe_v0 measures acc afc gratings via direct scanning
% This script creates an accumulated afc and then reads out the comb via
% scanning
...
%%%% afc parameters
nreads = 5; % number of scans to read out comb
readlength = 1000; % length of write sequence (us)

nwritepairs = 10; % number of pulse pairs.
psep = 0.3; %separation between write pulses
pdel = 100; %delay between pairs of pulses

writepulse_delays = [2 psep]; % reminder: delays are specified in reference to the last pulse.
writepulse_widths = [.15 .15];
...
waitdelays_exp = 1e-3;
waitdelays = (.1); %delay between burning pulse and read pulse (units determined by waitdelays_exp)
...
fgen2.ch1.pulses.timestep = 0.1; %only used for plotting
fgen2.ch2.pulses.timestep = 0.05; %vesitigial

fgen1.pulses.totaltime = pdel;
fgen2.ch1.pulses.totaltime = readpulse_widths;
fgen2.ch2.pulses.totaltime = readpulse_widths; %vestigial
...
fgen2.ch1.pulses.createpulses; % only used for plotting
...
%plotting a sawtooth
plot(hsp3,fgen1.pulses.tdata,fgen2.ch2.off + 0.5*fgen2.ch2.amp*sawtooth(2*pi*fgen1.pulses.tdata/readlength,rampsym))
