classdef srsfgen < handle
    ...
    methods
        ...
        function open(obj)
            obj.id = instrfind('Type', 'visa-gpib', 'RsrcName', 'GPIB0::19::0::INSTR', 'Tag', '');
...
%                     obj.ts = min([widths(widths>0),delays(delays>0)]);
                    if length(widths)==length(delays)
                        npulses = length(widths);
                        startstop_times = zeros(npulses,2);
                        startstop_times(1,1) = delays(1);
                        startstop_times(1,2) = startstop_times(1,1) + widths(1);
                        if npulses>1
                            for pno = 2:npulses
                                startstop_times(pno,1) = startstop_times(pno-1,2)+delays(pno);
                                startstop_times(pno,2) = startstop_times(pno,1)+widths(pno);
                            end
                        end
                    else
                        warning('oops. you need the same number of widths and delays');
                    end
                    % use varargin to specify the pulse heights.        
                    if ~isempty(varargin)
                        createpulses(obj,startstop_times,varargin{1});
                    else
                        createpulses(obj,startstop_times);
                    end
                case {'Gauss','Gaussian','gauss','gaussian'}                    
                    if length(widths)==length(delays)
                        ...
                        startstop_times = zeros(npulses,2);
                        startstop_times(1,1) = delays(1);
                        startstop_times(1,2) = widths(1);
                        if npulses>1
                            for pno = 2:npulses
                                startstop_times(pno,1) = startstop_times(pno-1,1)+delays(pno);
                                startstop_times(pno,2) = widths(pno);
                            end
                        end
                    else
                        warning('oops. you need the same number of widths and delays');
                    ...
                    % use varargin to specify the pulse heights.        
                    if ~isempty(varargin)
                        creategaussianpulses(obj,startstop_times,varargin{1});
                    else
                        creategaussianpulses(obj,startstop_times);
                    end
            end
        end 
...
% % %                  disp([num2str(length(obj.normydata)) ' points sent'])
% % %             else
...
% % %         end
        ...
        function sendpulses(obj)
            ...
            %selecting loaded data as output. could also copy to nonvolatile memory, but it's not really necessary
